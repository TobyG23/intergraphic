    ...
    <table>
        <tbody>
            <?php 
                $number_col = 1; 
                $hoz_fields = array();
                foreach ( $options['pm_hoz'] as $key => $hoz_field_id ): 
            ?>
            <tr>
                ...
                <?php endif; ?>
                <?php 
                    $hoz_field      = get_field_by_id( $hoz_field_id, $options["fields"] );
                    $number_col    *= count( $hoz_field['general']['attributes']["options"] );
                    $colspan        = 1;
                    $looptimes      = 1;
                    foreach ( $options['pm_hoz'] as $_key => $_hoz_field_id ){
                        $_hoz_field = get_field_by_id( $_hoz_field_id, $options["fields"] );
                        if( $_key > $key ){
            ...
            <?php
                $number_row = 1;
                $ver_fields = array();
                foreach ($options['pm_ver'] as $ver_field_id){
                    $ver_field = get_field_by_id( $ver_field_id, $options["fields"] );
                    $number_row *= count($ver_field['general']['attributes']["options"]);
                }
                for( $i=0; $i < $number_row; $i++ ){ ?>
            <tr>
            <?php foreach ($options['pm_ver'] as $key => $ver_field_id): ?>
                <?php 
                    $ver_field  = get_field_by_id( $ver_field_id, $options["fields"] );
                    $rowspan    = 1;
                    $looptimes  = 1;
                    foreach ($options['pm_ver'] as $_key => $_ver_field_id){
                        $_ver_field = get_field_by_id( $_ver_field_id, $options["fields"] );
                        if( $_key > $key ){
            ...
            ?>
                <td  ng-click="select_price_matrix(<?php echo( $i ); ?>, <?php echo( $j ); ?>)" class="{{options.price_matrix[<?php echo( $i ); ?>][<?php echo( $j ); ?>].class}}" ng-class="options.price_matrix[<?php echo( $i ); ?>][<?php echo( $j ); ?>].disable ? 'disabled' : ''" title="<?php esc_html_e('Choose', 'web-to-print-online-designer'); ?>">
                    <span ng-hide="options.price_matrix[<?php echo( $i ); ?>][<?php echo( $j ); ?>].disable" ng-bind-html="options.price_matrix[<?php echo( $i ); ?>][<?php echo( $j ); ?>].price | to_trusted"></span>
                </td>
            <?php } ?>
            </tr>
            <?php } ?>
