...
class NBD_Designer_API extends WP_REST_Controller {
    ...
    public function register_rest_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/status', array(
            'args' => array(
                'id' => array(
                    'description' => esc_html__( 'Unique identifier for the object.', 'web-to-print-online-designer' ),
                    'type'        => 'integer',
                    'required'    => true
                ),
                'enabled' => array(
                    'description' => esc_html__( 'Status enable for the designer object.', 'web-to-print-online-designer' ),
                    'type'        => 'string',
                    'required'    => false
                ),
                'featured' => array(
                    'description' => esc_html__( 'Status feature for the designer object.', 'web-to-print-online-designer' ),
                    'type'        => 'string',
                    'required'    => false
                )
            ),
        ...
        register_rest_route( $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/email' , array(
            'args' => array(
                ...
                'subject' => array(
                    'description' => esc_html__( 'Subject of the email.', 'web-to-print-online-designer' ),
                    'type'        => 'string',
                    'required'    => true,
                ),
                'message' => array(
                    'description' => esc_html__( 'Body of the email.', 'web-to-print-online-designer' ),
                    'type'        => 'string',
                    'required'    => true,
                ),
            ),
    ...
    public function update_designer( $request ){
        ...
        if ( empty( $designer->get_id() ) ) {
            return new WP_Error( 'no_designer_found', esc_html__( 'No designer found', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
        }
        ...
        $designer       = new NBD_Designer( $designer_id );
        $designer_data  = $this->prepare_item_for_response( $designer, $request, array( 'message' => esc_html__( 'Designer status has been updated!', 'web-to-print-online-designer' ) ) );
    ...
    public function update_designer_status( $request ){
        if ( !( isset( $request['enabled'] ) || isset( $request['featured'] ) ) ) {
            return new WP_Error( 'no_valid_status', esc_html__( 'Status parameter must be enabled or featured', 'web-to-print-online-designer' ), array( 'status' => 400 ) );
        }
        
        $designer_id = ! empty( $request['id'] ) ? $request['id'] : 0;
        
        if ( empty( $designer_id ) ) {
            return new WP_Error( 'no_designer_found', esc_html__( 'No designer found for updating status', 'web-to-print-online-designer' ), array( 'status' => 400 ) );
        }
        ...
        $data['message']    = esc_html__( 'Designer status has been updated!', 'web-to-print-online-designer' );
        $response           = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $data, $request ) );
        return $response;
    }
    public function batch_update( $request ){
        ...
        if ( empty( $params ) ) {
            return new WP_Error( 'no_item_found', esc_html__( 'No items found for bulk updating', 'web-to-print-online-designer' ), [ 'status' => 404 ] );
        }
        ...
        $response['message']    = esc_html__( 'Designers status have been updated!', 'web-to-print-online-designer' );
        return $response;
    }
    ...
    public function send_email_to_designer( $request ){
        ...
        $response['message']    = $response['success'] ? esc_html__( 'Email has been sent successfully.', 'web-to-print-online-designer' ) : esc_html__( 'Email has been sent failed', 'web-to-print-online-designer' );
