...
class NBD_Withdraw_API extends WP_REST_Controller {
        ...
        register_rest_route( $this->namespace, '/' . $this->base, array(
            array(
                ...
                'args'                => array_merge( $this->get_collection_params(),  array(
                    'status' => array(
                        'type'        => 'string',
                        'description' => esc_html__( 'Withdraw status', 'web-to-print-online-designer' ),
                        'required'    => false,
                    ),
                )),
        ...
        register_rest_route( $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/', array(
            ...
            array(
                ...
                'args'                => array(
                    ...
                    'note' => array(
                        'type'        => 'string',
                        'description' => esc_html__( 'Withdraw note', 'web-to-print-online-designer' ),
                        'required'    => false,
                    )
                ),
                'permission_callback' => array( $this, 'permission_check' ),
        ...
        if( isset( $request['status'] ) ){
            ...
            if( $status == 1 ){
                if ( round( nbdl_get_designer_balance( $result->user_id, false ), 2 ) < $result->amount ) {
                    return new WP_Error( 'not_approve_withdraw', esc_html__( 'Can not approve this withdraw request because the balance is less than the withdraw amount.', 'web-to-print-online-designer' ), array( 'status' => 400 ) );
                }
            ...
            if ( ! $update ) {
                return new WP_Error( 'note_not_udpated', esc_html__( 'Something wrong, Note not updated', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
            }

            $withdraw           = $wpdb->get_row( $wpdb->prepare("SELECT * from {$wpdb->prefix}nbdesigner_withdraw WHERE id = %d", $request_id ) );
            $withdraw->message  = esc_html__( 'Withdraw note has been updated!', 'web-to-print-online-designer' );
    ...
    public function delete_withdraw( $request ) {
        ...
        if ( !$withdraw_id ) {
            return new WP_Error( 'no_id', esc_html__( 'Invalid ID', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
        }
        ...
        if ( empty( $result->id ) ) {
            return new WP_Error( 'no_withdraw', esc_html__( 'No withdraw found for deleting', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
        }

        $deleted            = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}nbdesigner_withdraw WHERE id = %d", $withdraw_id ) );
        $result->message    = $deleted ? esc_html__( 'Withdraw has been delete successfully.', 'web-to-print-online-designer' ) : esc_html__( 'Withdraw has been delete failed', 'web-to-print-online-designer' );
    ...
    public function batch_update( $request ){
        ...
        $response['message']    = esc_html__( 'Withdraw status have been updated!', 'web-to-print-online-designer' );
        return $response;
    }
}