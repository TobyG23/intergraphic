...
class NBD_Design_API extends WP_REST_Controller {
        ...
        register_rest_route( $this->namespace, '/' . $this->base, array(
            array(
                ...
                'args'                => array_merge( $this->get_collection_params(),  array(
                    'status' => array(
                        'type'        => 'string',
                        'description' => esc_html__( 'Design status', 'web-to-print-online-designer' ),
                        'required'    => false,
                    ),
                )),
    ...
    public function delete_design( $request ) {
        ...
        if ( empty( $result->id ) ) {
            return new WP_Error( 'no_design', esc_html__( 'No design found for deleting', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
        }

        $deleted            = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}nbdesigner_templates WHERE id = %d", $design_id ) );
        $result->message    = $deleted ? esc_html__( 'Design has been delete successfully.', 'web-to-print-online-designer' ) : esc_html__( 'Design has been delete failed', 'web-to-print-online-designer' );
    ...
    public function batch_update( $request ){
        ...
        if ( empty( $params ) ) {
            return new WP_Error( 'no_item_found', esc_html__( 'No items found for bulk updating', 'web-to-print-online-designer' ), array( 'status' => 404 ) );
        }
        ...
        $response['message']    = esc_html__( 'Design status have been updated!', 'web-to-print-online-designer' );
        return $response;
    }
}