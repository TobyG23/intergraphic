...
if ( ! class_exists( 'NBDL_Withdraw_Cancelled' ) ) {

    class NBDL_Withdraw_Cancelled  extends WC_Email {

        public function __construct() {
            $this->id               = 'nbdl_email_withdraw_cancelled';
            $this->title            = esc_html__( 'NBDesigner Withdraw cancelled', 'web-to-print-online-designer' );
            $this->description      = esc_html__( 'These emails are sent to designer when a designer withdraw request is cancelled', 'web-to-print-online-designer' );
            $this->template_html    = 'launcher/emails/withdraw_cancelled.php';
            $this->template_plain   = 'launcher/emails/plain/withdraw_cancelled.php';
            $this->template_base    = NBDESIGNER_PLUGIN_DIR.'/templates/';

            add_action( 'nbdl_withdraw_request_cancelled', array( $this, 'trigger' ), 30, 2 );
        ...
        public function get_default_subject() {
            return esc_html__( '[{site_name}] Your withdrawal request was cancelled', 'web-to-print-online-designer' );
        }

        public function get_default_heading() {
            return esc_html__( 'Withdrawal request for {amount} is cancelled', 'web-to-print-online-designer' );
        }
...
return new NBDL_Withdraw_Cancelled();