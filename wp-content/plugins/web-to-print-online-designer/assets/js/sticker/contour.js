...
var Potrace = (function () {
    ...
    function Bitmap(w, h) {
        ...
        //this.data = new Uint16Array(this.arraybuffer);
    ...
    Bitmap.prototype.pointToIndex = function (pointOrX, y) {
        var _x = pointOrX,
            _y = y;
        ...
        if (!utils.between(_x, 0, this.w) || !utils.between(_y, 0, this.h)) {
            return -1;
        }
    ...
    Bitmap.prototype.index = function (i) {
        var point = new Point();
        if (utils.between(i, 0, this.size)) {
    ...
    Bitmap.prototype.copy = function (iterator) {
        var _bm = new Bitmap(this.w, this.h),
            iteratorPresent = typeof iterator === 'function',
            i;
        for (i = 0; i < this.size; i++) {
            _bm.data[i] = iteratorPresent ? iterator(this.data[i], i) : this.data[i];
        }
        return _bm;
    };
    ...
    function loadData(imgData, width, height) {
        loadCanvas(imgData, width, height);
        setTimeout(function () {
            createShade();
        }, 100);
        setTimeout(function () {
    ...
    function loadCanvas(imgdataObj, width, height) {
        imgCanvas.width = width;
        imgCanvas.height = height;
        var ctx = imgCanvas.getContext('2d');
        ctx.putImageData(imgdataObj, info.margin, info.margin);
    }
    ...
    function loadBm() {
        ...
        for (i = 0, j = 0; i < l; i += 4, j++) {
            ...
            var opacity = imgdataobj.data[i + 3] / 255,
                r = 255 + (imgdataobj.data[i + 0] - 255) * opacity,
                g = 255 + (imgdataobj.data[i + 1] - 255) * opacity,
                b = 255 + (imgdataobj.data[i + 2] - 255) * opacity;
            bm.data[j] = utils.luminance(r, g, b);
        }
    ...
    function createShade() {
        var shade = [],
            ...
            imgDataCopy = imgdataobj;

        function setColorAtIndex(index, r, g, b, a) {
            imgDataCopy.data[index] = r;
            imgDataCopy.data[index + 1] = g;
            imgDataCopy.data[index + 2] = b;
            imgDataCopy.data[index + 3] = a;
        }

        var l = imgdataobj.data.length, i, j;
        for (i = 0, j = 0; i < l; i += 4, j++) {
        ...
        function getPixelColor(x, y) {
            if (x < 0 || y < 0 || x >= width || y >= height) {
                ...
                return [imgDataCopy.data[index], imgDataCopy.data[index + 1], imgDataCopy.data[index + 2], imgDataCopy.data[index + 3]];
            }
        }

        function compareTwoColos(color1, color2) {
        ...
        var targetColor = getPixelColor(0, 0);
        ...
        while (pixelsToCheck.length > 0) {
            ...
            if (compareTwoColos(currentColor, targetColor)) {
                setColorAtIndex(index, 255, 0, 0, 255);
        ...
        for (i = 0, j = 0; i < l; i += 4, j++) {
            var currentColor = [imgDataCopy.data[i], imgDataCopy.data[i + 1], imgDataCopy.data[i + 2], imgDataCopy.data[i + 3]],
                floodColor = [255, 0, 0, 255];
            if (!compareTwoColos(currentColor, floodColor)) {
                setColorAtIndex(i, 0, 0, 0, 255);
            } else {
                setColorAtIndex(i, 0, 0, 0, 0);
            }
        ...
        shade.forEach(function (index) {
            var __point = indexToPixel(index);
            var x = __point[0],
                y = __point[1],
                isContour = false, nIndex;
            ...
            for (let i = x - 1; i <= x + 1; i++) {
                ...
                for (let j = y - 1; j <= y + 1; j++) {
                    ...
                    if (imgDataCopy.data[nIndex + 3] <= threshold) {
                        contour.push(index);
                        isContour = true;
                        break;
                    }
        ...
        contour.forEach(function (index) {
            var margin = info.margin,
                __point = indexToPixel(index),
                x = __point[0],
                y = __point[1];
            ...
            for (let i = x - margin; i <= x + margin; i++) {
                for (let j = y - margin; j <= y + margin; j++) {
                    if (margin * margin >= ((x - i) * (x - i) + (y - j) * (y - j))) {
                        nIndex = (width * j + i) * 4;
                        imgDataCopy.data[nIndex] = 0;
                        imgDataCopy.data[nIndex + 1] = 0;
                        imgDataCopy.data[nIndex + 2] = 0;
                        imgDataCopy.data[nIndex + 3] = 255;
                    }
                }
            }
        });
    ...
    function bmToPathlist() {
        ...
        var bm1,
            ...
            blackOnWhite = info.blackOnWhite,
            currentPoint = new Point(0, 0),
            path;

        threshold = 128;
        bm1 = bm.copy(function (lum) {
    ...
    function getSVG(size, opt_type, onlyPath) {
        ...
        for (i = 0; i < len; i++) {
            c = pathlist[i].curve;
            //svg += path(c);
            pathcmd += utils.renderCurve(c, 1);
        }

        if( onlyPath ) return {
            pathcmd: pathcmd,
            needResize: info.needResize
        };

        var svg = '<svg version="1.1" width="' + w + '" height="' + h + '"' + ' viewBox="0 0 ' + w + ' ' + h + '" ' + ' xmlns="http://www.w3.org/2000/svg">';
        if (opt_type === "curve") {
