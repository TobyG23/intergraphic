...
var apiPre = NBDESIGNCONFIG.printcart_api_url;

if( apiPre.slice(-1) != '/' ) {
    apiPre = NBDESIGNCONFIG.printcart_api_url + '/';
}

// var printcart_api_enable = NBDESIGNCONFIG.printcart_api_enable;
var printcart_api_enable = true;

var apiAuthBearer = {
    'X-PrintCart-Unauth-Token': NBDESIGNCONFIG.printcart_unauth_token
};
...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.initSettings = function(){
        ...
        $scope.isMobileDevice = checkMobileDevice();
        ...
        if( printcart_api_enable ) {
            $scope.resource.clipart = {filter: {perPage: 20, currentPage: 1, total: 0, currentCat: {}, changeFilter: false}, data: [], onload: false, init: true, filteredArts : [], nextPage: {}};
        }
        ...
        if( printcart_api_enable ) {
            $scope.$watchCollection('resource.clipart.filter', function(newVal, oldVal){
                ...
                var currentCat = $scope.resource.clipart.filter.currentCat.id ? $scope.resource.clipart.filter.currentCat.id : 'all';
                $scope.resource.clipart.filteredArts = [];
                if(angular.isDefined($scope.resource.clipart.data.cat_data[currentCat])) {
                    $scope.resource.clipart.filteredArts = filterArtFilter($scope.resource.clipart.data.cat_data[currentCat], $scope.resource.clipart.filter);
                }
                if(newVal.search != oldVal.search){
                    $timeout(function() {
    ...
    $scope.initSearchTempalte = function(){
        if( angular.isDefined( $scope.settings.template_tags ) && $scope.settings.template_tags.length ){
            var source = $scope.settings.template_tags.map(function( tag ){
                return {
                    label: tag.name,
                    value: tag.id
                };
    ...
    $scope.init = function() {
        ...
        _document.on('webkitfullscreenchange mozfullscreenchange fullscreenchange MSFullscreenChange', function(){
    ...
    $scope.getResource = function(type, container, callback){
        ...
        if( (type == 'clipart' || type === 'myClipart') && printcart_api_enable ) {
            ...
            var url = apiPre + 'clipart-storages/default?limit=99';
            var headers = apiAuthBearer;
            if( type === 'myClipart') {
                url = apiPre + 'clipart-storages?limit=99';
            }
            $http({
                method: "GET",
                url: url,
            ...
            case 'myClipart':
                if( printcart_api_enable ){
                    ...
                    $http({
                        ...
                        $scope.resource[type].data.cat_data['all'] = response.data.data;
                        if(response.data.links.next) {
                            $scope.resource[type].nextPage['all'] = response.data.links.next+ '&limit=' + $scope.resource[type].filter.perPage;
                        } else {
                            $scope.resource[type].nextPage['all'] = null;
                        }
                        var firstEl = {
                            name: 'All',
                            id: 'all',
                        }
                        $scope.resource[type].data.cat.unshift(firstEl);
                        if( $scope.resource[type].data.cat.length ){
                            $scope.resource[type].filter.currentCat = $scope.resource[type].data.cat[0];
                            $scope.resource[type].filter.total = response.data.data.length;
                        }else{
                            $scope.resource[type].filter.currentCat = -1;
                            $scope.resource[type].filter.currentPage = 0;
                            $scope.resource[type].filter.total = 0;
                        }
    ...
    $scope.changeCatNew = function( type , cat){
        ...
        if(angular.isUndefined($scope.resource[type].data.cat_data[currentCat]) ) {
            ...
            $http({
                ...
                if(response.data.links.next) {
                    $scope.resource.clipart.nextPage[currentCat] = response.data.links.next+ '&limit=' + $scope.resource.clipart.filter.perPage;
                } else {
                    $scope.resource.clipart.nextPage[currentCat] = null;
                }
                switch(type){
                    case 'clipart':
                    case 'myClipart':
                ...
                angular.forEach($scope.settings.product_data.product, function(side, key){
                    ...
                    if(data.color.bg_type == 'c'){
                        ...
                        if( angular.isDefined(data.page) && angular.isDefined( data.page.list_page ) ){
                            ...
                            if( angular.isDefined(data.color.bg_image) && angular.isDefined(data.color.bg_image[key]) ) side.img_src = data.color.bg_image[key];
                        }
                    }
                });
            }
    ...
    $scope.renderMasonryList = function(type, container, item, scrollContainer, init){  
        imagesLoaded( jQuery(container), function() {
            ...
            $timeout(function(){
                if(type != 'clipart' && printcart_api_enable) {
    ...
    $scope.scrollLoadMore = function(container, type){
        ...
        if(type == 'clipart') {
            ...
            if(type == 'typography' && angular.isDefined(NBDESIGNCONFIG.nbdesigner_limit_typo) && NBDESIGNCONFIG.nbdesigner_limit_typo > 0) {
                $scope.resource[type].onload = true;
                return;
            }
    ...
    $scope.processAreaDesignShape = function(index){
        ...
        $timeout(function(){
            ...
            var oldDefs = svg.getElementsByTagName('defs')[0];
            if( oldDefs ){
        ...
        jQuery('.nbd-eyedropper-inner-wrap').off('mousedown').on('mousedown', function(evt){
            ...
            if( canvasLeft > 0 && canvasTop > 0 && canvasLeft <= _canvas.width && canvasTop <= _canvas.height ){
                ...
                if( global ){
                    $scope.globalPicker.color = color;
                    $scope.addColor( $scope.globalPicker.color );
                    $scope.selectGlobalPicker( $scope.globalPicker.color );
                }else{
                    $scope.currentColor = color;
                    $scope.addColor();
                    $scope.changeFill(color);
    ...
    $scope.generateStickerCutline = function( onSaveDesign ){
        ...
        _canvas = _stage.canvas,
        width = Math.floor( _canvas.width ) * ratio,
        height = Math.floor( _canvas.height ) * ratio,
        bleedTB = ratio * _stage.config.bleed_tb,
        bleedLR = ratio * _stage.config.bleed_lr,
        ...
        function _showStickerPreview(){
            $timeout(function(){
                ...
                var imgdataObj = ctx.getImageData(margin, margin, width - 2 * margin, height - 2 * margin);
    ...
    filters.CustomFilter = createClass(filters.BaseFilter, {
        ...
        applyTo2d: function(options) {
            ...
            return new Promise( function( resolve, reject ) {
                ...
                var img = new Image();
                var url = "data:image/svg+xml;charset=utf-8," + encodeURIComponent( filterdSvg );
    ...
    return {
        ...
        link: function(scope, element) {
            $timeout(function() {
                ...
                jQuery(element).on('paste', function(e){
