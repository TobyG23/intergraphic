jQuery( function ( $ ) {
    ...
    $(document).ready(function () {
        //Order functions
        $('#nbdq_submit_button').on('click', function (e) {
            e.preventDefault();
            $('#_nbdq_safe_submit_field').val('send_quote');

            $(this).closest('form').submit();
        });
    });

    var $datepicker   = $( '.nbdq-plugin-fw-datepicker' ),
        $colorpicker  = $( '.nbdq-plugin-fw-colorpicker' ),
        $upload       = {
            imgPreviewHandler  : '.nbdq-plugin-fw-upload-img-preview',
            uploadButtonHandler: '.nbdq-plugin-fw-upload-button',
            imgUrlHandler      : '.nbdq-plugin-fw-upload-img-url',
            resetButtonHandler : '.nbdq-plugin-fw-upload-button-reset',
            imgUrl             : $( '.nbdq-plugin-fw-upload-img-url' )
        },
        $wpAddMedia   = $( '.add_media' ),
        $imageGallery = {
            sliderWrapper: $( '.nbdq-plugin-fw .image-gallery ul.slides-wrapper' ),
            buttonHandler: '.nbdq-plugin-fw .image-gallery-button'
        },
        $onoff        = $( '.nbdq-plugin-fw-onoff-container span' ),
        $sidebars     = $( '.nbdq-plugin-fw-sidebar-layout' ),
        $slider       = $( '.nbdq-plugin-fw .nbdq-plugin-fw-slider-container .ui-slider-horizontal' ),
    ...
    if ( typeof wp !== 'undefined' && typeof wp.media !== 'undefined' ) {
        ...
        $( document ).on( 'click', $upload.uploadButtonHandler, function ( e ) {
            ...
            custom_uploader.on( 'select', function () {
                ...
                // Save the id of the selected element to an element which name is the same with a suffix "-nbdq-attachment-id"
                if ( $( "#" + id + "-nbdq-attachment-id" ) ) {
                    $( "#" + id + "-nbdq-attachment-id" ).val( attachment.id );
                }
    ...
    $sidebars.each( function () {
        ...
        $images.on( 'click', function () {
            var $container = $( this ).closest( '.nbdq-plugin-fw-sidebar-layout' ),
                $left      = $container.find( '.nbdq-plugin-fw-sidebar-layout-sidebar-left-container' ),
                $right     = $container.find( '.nbdq-plugin-fw-sidebar-layout-sidebar-right-container' ),
                type       = $( this ).data( 'type' );
    ...
    $( document ).on( 'click', '.nbdq-plugin-fw-select-all', function () {
        var $targetSelect = $( '#' + $( this ).data( 'select-id' ) );
        $targetSelect.find( 'option' ).prop( 'selected', true ).trigger( 'change' );
    } );

    $( document ).on( 'click', '.nbdq-plugin-fw-deselect-all', function () {
        var $targetSelect = $( '#' + $( this ).data( 'select-id' ) );
    ...
    /** Select Images */
    $( document ).on( 'click', '.nbdq-plugin-fw-select-images__item', function () {
        var item    = $( this ),
            key     = item.data( 'key' ),
            wrapper = item.closest( '.nbdq-plugin-fw-select-images__wrapper' ),
            items   = wrapper.find( '.nbdq-plugin-fw-select-images__item' ),
            select  = wrapper.find( 'select' ).first();

        if ( select.length ) {
            select.val( key );
            items.removeClass( 'nbdq-plugin-fw-select-images__item--selected' );
            item.addClass( 'nbdq-plugin-fw-select-images__item--selected' );
        }
    } );
} );