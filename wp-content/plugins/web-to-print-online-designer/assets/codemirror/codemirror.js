...
window.CodeMirror = (function() {
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        if (document.activeElement != focused && focused.offsetHeight)
            focused.focus();
    ...
    function restartBlink(cm) {
        ...
        display.blinker = setInterval(function() {
            if (!display.cursor.offsetHeight)
                return;
            ...
            if (doc.frontier >= cm.display.showingFrom) { // Visible
                ...
                if (ischange) {
                    if (prevChange && prevChange.end == doc.frontier)
                        ...
                        changed.push(prevChange = {start: doc.frontier, end: doc.frontier + 1});
                }
    ...
    function measureLine(cm, line) {
        ...
        if (cache.length == 16)
            cache[++display.measureLineCachePos % 16] = memo;
        else
            cache.push(memo);
        return measure;
    }
    ...
    function coordsCharInner(cm, lineObj, lineNo, x, y) {
        ...
        function getX(ch) {
            ...
            if (innerOff > sp.bottom)
                return Math.max(0, sp.left - cWidth);
            else if (innerOff < sp.top)
                return sp.left + cWidth;
            else
                wrongLine = false;
            return sp.left;
        }
    ...
    function allKeyMaps(cm) {
        ...
        if (cm.options.extraKeys)
            maps.unshift(cm.options.extraKeys);
        return maps;
    }
    ...
    function computeSelAfterChange(doc, change, hint) {
        ...
        if (hint == "start")
            return {anchor: change.from, head: change.from};

        var end = changeEnd(change);
        if (hint == "around")
            return {anchor: change.from, head: end};
        if (hint == "end")
    ...
    function moveVisually(line, start, dir, byUnit) {
        ...
        for (var i = 0; i < bidi.length; ++i) {
            ...
            if ((part.from < start && part.to > start) ||
                    (sticky && (part.from == start || part.to == start)))
                break;
        }
                ...
                if (countsAsLeft.test(types[i])) {
                    ...
                    for (var j = pos; j < i; ) {
                        if (countsAsNum.test(types[j])) {
                            if (pos < j)
                                order.splice(at, 0, {from: pos, to: j, level: 1});
                            var nstart = j;
                    ...
                    if (pos < i)
                        order.splice(at, 0, {from: pos, to: i, level: 1});
                }
            }
